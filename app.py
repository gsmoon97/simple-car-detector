...
# initialize our Flask application and the detector
app = Flask(__name__)
detector = None
...
def draw_boxes(image, boxes, class_names, scores, max_boxes=10, min_score=0.1):
    ...
    for i in range(min(boxes.shape[0], max_boxes)):
        # only detect cars
        if scores[i] >= min_score and class_names[i].decode("ascii") == "Car":
...
def upload_image():
    ...
    if file and allowed_file(file.filename):
        ...
        image_with_boxes = run_detector(input_path)
        iwb = Image.fromarray(image_with_boxes)
        iwb.save(output_path)
        #print('upload_image filename: ' + filename)
        flash('Image successfully uploaded and displayed below')
        return render_template('upload.html', filename="result_" + filename)
    else:
